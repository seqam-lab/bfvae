...
class Solver(object):
    ...
    def __init__(self, args):
        ...
        if self.viz_on:
            ...
            self.win_id = dict( 
                ...
                kl_alpha='win_kl_alpha', rvS='win_rvS', rvH='win_rvH' )
    ...
    def train(self):
        ...
        for iteration in range(start_iter, self.max_iter+1):
            ...
            # prior alpha params
            a, b, rvlogit, rv = self.prior_alpha()
            ...
            vae_loss = loss_recon + loss_kl + loss_kl_alpha + \
                       self.gamma*loss_tc + self.etaS*loss_sparse + \
                       self.etaH*loss_entropy
            ...
            if iteration % self.print_iter == 0:
                prn_str = ( '[iter %d (epoch %d)] vae_loss: %.3f | ' + \
                  ...
                  ( iteration, epoch, vae_loss.item(), dis_loss.item(), 
                    ...
                    loss_tc.item(), loss_sparse.item(), loss_entropy.item() )
                prn_str += '\n    rv = {}'.format(
